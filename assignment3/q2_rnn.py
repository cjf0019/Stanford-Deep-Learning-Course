...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        rawsent = sentence
        rawlabels = labels
        padding = max_length - len(rawsent)
        if padding == 0:
            ret.append((rawsent, rawlabels, [i != 4 for i in rawlabels]))
        elif padding < 0:
            ret.append((rawsent[:max_length], rawlabels[:max_length], \
                        [i != 4 for i in rawlabels[:max_length]]))
        else:
            for i in range(padding):
                rawsent.append(zero_vector)
                rawlabels.append(zero_label)
            ret.append((rawsent, rawlabels, [i != 4 for i in rawlabels]))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, mask_batch, labels_batch=None, dropout=1):
        ...
        if labels_batch is not None:
            feed_dict = {self.input_placeholder: inputs_batch, self.mask_placeholder: \
                         mask_batch, self.labels_placeholder: labels_batch, \
                         self.dropout_placeholder: dropout}
        else:
            feed_dict = {self.input_placeholder: inputs_batch, self.mask_placeholder: \
                         mask_batch, self.dropout_placeholder: dropout}
        ### END YOUR CODE
        return feed_dict
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable('U', shape=(self.config.hidden_size, 
                            self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.Variable(tf.zeros(self.config.n_classes), name='b2')
        h = tf.Variable(tf.zeros(self.config.hidden_size), name='h')
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=tf.boolean_mask(preds, self.mask_placeholder), \
    ...
    def consolidate_predictions(self, examples_raw, examples, preds):
        ...
        for i, (sentence, labels) in enumerate(examples_raw):
            ...
            labels2= [j for j in labels if j != 4]
            assert len(labels_) == len(labels2)
