...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        if len(sentence) != len(labels):
            raise ValueError
        nfeatures = len(sentence[0])
        flatsentence = window_size*[i for i in start] + [i for j in sentence for i in j] + window_size*[i for i in end]
        for pos, word in enumerate(sentence):
            windowed_data.append((flatsentence[nfeatures*pos:(pos+2*window_size+1)*nfeatures], labels[pos]))
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', shape=(self.config.n_window_features*self.config.embed_size,
                            self.config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
